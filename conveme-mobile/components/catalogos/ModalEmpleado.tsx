...
import { X, Save, Users, MapPin, Briefcase, Check, Search, Mail, Phone, User } from 'lucide-react-native';
...
export default function ModalEmpleado({ isOpen, onClose, onSave, empleadoAEditar }: Props) {
    ...
    const resetForm = () => {
        ...
        setEstadoId(null); setMunicipioId(null); setUsuarioId(null);
        setMunicipios([]); setErrors({});
    };

    const cargarCatalogos = async () => {
        try {
            const [dataEst, dataUsr] = await Promise.all([getEstados(), getUsuariosParaSelect()]);
            setEstados(dataEst); setUsuariosLista(dataUsr);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const payload: any = {
                ...
                telefono: telefono.trim() || undefined, puesto: puesto.trim() || undefined,
                calle_y_numero: calleYNumero.trim() || undefined, colonia: colonia.trim() || undefined,
                codigo_postal: codigoPostal.trim() || undefined,
                municipio_id: municipioId || undefined,
            };
            if (!esEdicion) payload.usuario_id = usuarioId;
    ...
    const usuarioSel = usuariosLista.find(u => u.id_usuario === usuarioId);
    ...
    return (
        <Modal transparent visible={isOpen} animationType="fade" onRequestClose={onClose}>
                ...
                <View style={s.modal}>
                    <View style={s.header}>
                        <View style={s.headerLeft}>
                            <View style={[s.headerIcon, { backgroundColor: esEdicion ? 'rgba(204,85,255,0.12)' : Colors.verdeFondo, borderColor: esEdicion ? 'rgba(204,85,255,0.2)' : 'rgba(6,214,160,0.2)' }]}>
                                <Users size={20} color={esEdicion ? Colors.moradoAccento : Colors.verdeExito} />
                            </View>
                            <View>
                                <Text style={s.headerTitle}>{esEdicion ? 'Editar empleado' : 'Nuevo empleado'}</Text>
                                <Text style={s.headerSub}>{esEdicion ? 'Modifica los datos' : 'Registra un nuevo empleado'}</Text>
                            </View>
                        </View>
                    ...
                    {step === 'form' ? (
                        <>
                            <ScrollView style={s.body} contentContainerStyle={{ gap: 14 }}>
                                ...
                                <View style={s.section}>
                                    <View style={s.sectionHead}><Briefcase size={13} color={Colors.moradoAccento} /><Text style={[s.sectionLabel, { color: Colors.moradoAccento }]}>Datos personales</Text></View>
                                    <View><View style={s.labelRow}><User size={11} color={Colors.moradoAccento} /><Text style={s.label}>Nombre completo</Text></View>
                                        <TextInput style={s.input} placeholder="Ej. Ana Torres" placeholderTextColor="#b9a0d4" value={nombreCompleto} onChangeText={setNombreCompleto} editable={!loading} />
                                        {errors.nombre && <Text style={s.errorText}>{errors.nombre}</Text>}
                                    </View>
                                    <View style={s.grid}>
                                        <View style={s.gridItem}><View style={s.labelRow}><Mail size={11} color={Colors.moradoAccento} /><Text style={s.label}>Email</Text></View>
                                        ...
                                        <View style={s.gridItem}><View style={s.labelRow}><Phone size={11} color={Colors.moradoAccento} /><Text style={s.label}>Telefono</Text></View>
                                            <TextInput style={s.input} placeholder="10 digitos" placeholderTextColor="#b9a0d4" value={telefono} onChangeText={t => setTelefono(t.replace(/\D/g, ''))} maxLength={10} editable={!loading} keyboardType="phone-pad" />
                                        </View>
                                    </View>
                                    <View><View style={s.labelRow}><Briefcase size={11} color={Colors.moradoAccento} /><Text style={s.label}>Puesto</Text></View>
                                        <TextInput style={s.input} placeholder="Ej. Coordinador Ventas" placeholderTextColor="#b9a0d4" value={puesto} onChangeText={setPuesto} editable={!loading} />
                                    </View>
                                    {!esEdicion && (
                                        <View><View style={s.labelRow}><User size={11} color={Colors.moradoAccento} /><Text style={s.label}>Usuario del sistema</Text></View>
                                            <TouchableOpacity style={s.selectBtn} onPress={() => { setShowUsuarioPicker(true); setSearchPicker(''); }}>
                                                <Text style={usuarioSel ? s.selectText : s.selectPlaceholder}>{usuarioSel ? `ID:${usuarioSel.id_usuario} - ${usuarioSel.username}` : 'Selecciona...'}</Text>
                                            </TouchableOpacity>
                                ...
                                {/* Direccion */}
                                <View style={s.section}>
                                    <View style={s.sectionHead}><MapPin size={13} color={Colors.verdeExito} /><Text style={[s.sectionLabel, { color: Colors.verdeExito }]}>Direccion (opcional)</Text></View>
                                    <View><View style={s.labelRow}><MapPin size={11} color={Colors.moradoAccento} /><Text style={s.label}>Calle y numero</Text></View>
                                        <TextInput style={s.input} placeholder="Ej. Av. Revolucion 245" placeholderTextColor="#b9a0d4" value={calleYNumero} onChangeText={setCalleYNumero} editable={!loading} />
                                    </View>
                                    <View style={s.grid}>
                                        <View style={s.gridItem}><Text style={s.label}>Colonia</Text><TextInput style={s.input} placeholder="Colonia" placeholderTextColor="#b9a0d4" value={colonia} onChangeText={setColonia} editable={!loading} /></View>
                                        <View style={s.gridItem}><Text style={s.label}>C.P.</Text><TextInput style={s.input} placeholder="5 digitos" placeholderTextColor="#b9a0d4" value={codigoPostal} onChangeText={t => setCodigoPostal(t.replace(/\D/g, ''))} maxLength={5} editable={!loading} keyboardType="number-pad" /></View>
                                    </View>
                                    <View style={s.grid}>
                            ...
                            <View style={s.footer}>
                                <TouchableOpacity style={[s.saveBtn, esEdicion && s.saveBtnEdit]} onPress={handleSubmit} disabled={loading} activeOpacity={0.8}>
                                    {loading ? <ActivityIndicator color={Colors.amarilloAccento} /> : (
                                        <><Save size={16} color={esEdicion ? Colors.blanco : Colors.amarilloAccento} /><Text style={[s.saveBtnText, esEdicion && { color: Colors.blanco }]}>{esEdicion ? 'Actualizar' : 'Guardar'} empleado</Text></>
                                    )}
                                </TouchableOpacity>
                            </View>
                        </>
                        ...
                        <View style={s.successWrap}>
                            ...
                            <Text style={s.successTitle}>{esEdicion ? 'Empleado actualizado!' : 'Empleado registrado!'}</Text>
            ...
            {renderPickerModal(showUsuarioPicker, () => setShowUsuarioPicker(false), usuariosLista, 'id_usuario', i => `ID:${i.id_usuario} - ${i.username}`, usuarioId, setUsuarioId, 'Usuario')}
        </Modal>
    );
}
...
const s = StyleSheet.create({
    ...
    sectionLabel: { fontWeight: '800', fontSize: 9.5, letterSpacing: 1.4, textTransform: 'uppercase' },
    labelRow: { flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: 5 },
    label: { fontWeight: '700', fontSize: 10, letterSpacing: 1, textTransform: 'uppercase', color: Colors.azulMarino, marginBottom: 5 },
