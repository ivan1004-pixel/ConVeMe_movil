...
import { X, Save, Tags, Check, Trash2, AlertTriangle } from 'lucide-react-native';
...
export default function ModalCategoria({ isOpen, onClose, onSave, categoriaAEditar }: Props) {
                ...
                <View style={s.modal}>
                    ...
                    <View style={s.header}>
                        <View style={s.headerLeft}>
                            <View style={[s.headerIcon, { backgroundColor: esEdicion ? 'rgba(204,85,255,0.12)' : Colors.verdeFondo, borderColor: esEdicion ? 'rgba(204,85,255,0.2)' : 'rgba(6,214,160,0.2)' }]}>
                                <Tags size={20} color={esEdicion ? Colors.moradoAccento : Colors.verdeExito} />
                            </View>
                            <View>
                                <Text style={s.headerTitle}>{esEdicion ? 'Editar categoria' : 'Nueva categoria'}</Text>
                                <Text style={s.headerSub}>{esEdicion ? 'Modifica el nombre' : 'Crea una categoria de producto'}</Text>
                            </View>
                        </View>
                    ...
                    {step === 'form' ? (
                        <ScrollView style={s.body} contentContainerStyle={{ gap: 18 }}>
                            <View>
                                <View style={s.labelRow}><Tags size={13} color={Colors.moradoAccento} /><Text style={s.label}>Nombre de Categoria</Text></View>
                                <TextInput
                                    style={s.input} placeholder="Ej. Pines, Stickers..."
                                    placeholderTextColor="#b9a0d4" value={nombre}
                                    onChangeText={setNombre} editable={!loading}
                                    data-testid="modal-categoria-nombre"
                                />
                            </View>
                            <TouchableOpacity
                                style={[s.saveBtn, esEdicion && s.saveBtnEdit]}
                                onPress={handleSubmit} disabled={loading || !nombre.trim()}
                                activeOpacity={0.8} data-testid="modal-categoria-guardar"
                            >
                                {loading ? <ActivityIndicator color={Colors.amarilloAccento} /> : (
                                    <><Save size={16} color={esEdicion ? Colors.blanco : Colors.amarilloAccento} /><Text style={[s.saveBtnText, esEdicion && { color: Colors.blanco }]}>{esEdicion ? 'Actualizar' : 'Guardar'} categoria</Text></>
                                )}
                            </TouchableOpacity>
                        </ScrollView>
                    ) : (
                        <View style={s.successWrap}>
                            ...
                            <Text style={s.successTitle}>{esEdicion ? 'Categoria actualizada!' : 'Categoria registrada!'}</Text>
