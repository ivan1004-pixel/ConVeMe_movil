...
export const createEvento = async (input: any) => {
    const query = `mutation CreateEvento($input: CreateEventoInput!) { createEvento(createEventoInput: $input) { id_evento nombre } }`;
...
export const updateEvento = async (input: any) => {
    const query = `mutation UpdateEvento($input: UpdateEventoInput!) { updateEvento(updateEventoInput: $input) { id_evento nombre } }`;
    const { data } = await convemeApi.post('', { query, variables: { input } });
    if (data.errors) throw new Error(data.errors[0].message);
...
export const deleteEvento = async (id: number) => {
    const query = `mutation RemoveEvento($id: Int!) { removeEvento(id_evento: $id) { id_evento } }`;
